function [ obj ] = DAQpkg( obj, hobj, event )
...
if strcmp(obj.status,'standing') && strcmp(get(hobj,'String'),'ImgSeq')
    set(hobj,'String','Stop ImgSeq');
    
%     for itmp=1:strcmp(obj.movie_mode,'zstack_plain')
        obj.status = 'ImgSeq Running';
        pause(.01)
...
%                 set(hobj,'String','100ms Imgs Saving')
            ...
            %% 10 images at 50 ms exposure rate (10 fps)
            for cc = 1:1
            %Change illumination mode to fluorescence
            fprintf(obj.sola,'%s',char([hex2dec('4F') hex2dec('7D') hex2dec('50')])); % Enable all channels
            obj.nidaq2.outputSingleScan([0 0]);
